...
-- Original Credit to: https://github.com/h3xcat/gmod-luabsp

---------------------------------------
--     Utilities | luabsp Library    --
---------------------------------------
...
do
    ...
    function LuaBSP.new(mapname)
        ...
        local bsp = setmetatable({}, LuaBSP)
        local fl = bsp:GetMapFileHandle(mapname)

        local ident = fl:Read(4) -- BSP file identifier
        if ident ~= "VBSP" then error("[LuaBSP] Invalid file header: " .. ident) return end

        bsp.version = fl:ReadLong() -- BSP file version
        bsp.lumps = {} -- lump directory array
        ...
        bsp.map_revision = fl:ReadLong() -- the map's revision (iteration, version) number

        --[[
        for i=0, HEADER_LUMPS-1 do
            local lump_data = bsp.lumps[i]
...
function Star_Trek.Util:LoadMap(map_name)
